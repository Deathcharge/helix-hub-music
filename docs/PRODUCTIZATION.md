...
### Observed candidate acceptance

The checks below supersede that initial pending-acceptance note. Logical commits are `51ba506` (initialization/stage cleanup), `cb26aec` (release tooling, tests, CI and guides), and `27d8ce2` (synchronized browser regression setup, current pilot installer tools and package-readable documentation links), followed by acceptance/documentation clarification at `2750d0b`. Application source is unchanged after `cb26aec`; the later release-tool startup fix is recorded separately below. Earlier commands without `-I` are historical test evidence, not current instructions. The pre-fix `candidate-041-first`, `candidate-041-final`, `candidate-041-reviewed`, and CI run 33413072882 bundles are superseded for evaluation and retained only as diagnostics; use a new post-fix bundle and the current guide.

- `output/playwright/lifecycle-env/Scripts/python.exe -m pytest`: **198 passed, one Windows FIFO skip**, **92.54%** application branch-aware coverage. The same command from the extracted `cb26aec` sdist passed **198/one skip** in 39.55 seconds. Tooling tests are included in the test count, not in the application coverage percentage. A FastAPI/Starlette TestClient deprecation warning remains upstream; it did not fail tests and was not suppressed.
- `python -m ruff check samsarix_workspace tests e2e tools`, `python -m ruff format --check samsarix_workspace tests e2e tools`, `python -m mypy samsarix_workspace tools`, `node --check samsarix_workspace/static/app.js`, and `git diff --check`: passed using the local development environment; 26 formatted Python files and 13 type-checked application/tool files.
- `output/playwright/release-tools/Scripts/python.exe tools/release_evidence.py build output/releases/candidate-041-first`: passed from clean `cb26aec4141175d7ae8a73c1cfe8115677d58329`. Repeated the complete build at clean `27d8ce2c8941dd04f8ad85fed3668f356fcf9844` into `output/releases/candidate-041-final`: passed, including Twine, runtime-only smoke, hash-locked reinstall, schema/inventory/repeat-generation checks and final verifier.
- `py -3.11 tools/release_evidence.py verify output/releases/candidate-041-final` and the copied standalone verifier against the first bundle: passed with **seven verified artifacts**. The `27d8ce2` Windows/Python 3.11.9 wheel SHA-256 is `2b684e343307a824a366e6772b20fc3683303f1f24be9cd05d8265419286585d`; sdist SHA-256 is `f7d1bef2d9357962fb91fbd1552f6f56686965329dcd6673585b24e99eca7152`. These identify that exact snapshot, not later builds or publisher identity. The final manifest retains all artifact sizes/hashes.
- Wheel inspection found 21 files with no tests or maintainer tools. All 14 application files, including UI assets, were byte-identical between those two wheels. CycloneDX contained 13 dependency components plus the AGPL application root; declared dependency licenses were MIT, BSD-3-Clause or PSF-2.0. Inventory privacy sentinel checks found no local file URL, installation location or host username in the SBOM. This is metadata inspection, not a legal determination.
- A separate fresh Python 3.11.9 venv installed from the final bundle directory using the guide's pinned installer update and `pip install --require-hashes --only-binary=:all: --find-links . -r runtime-requirements.txt`. `python -m samsarix_workspace init` and `pip check` passed. `python -I tools/release_smoke.py 0.4.1` also passed using that installed interpreter, exercising real loopback UI/save/History/Trash and shutdown without developer dependencies.
- The first pilot environment inherited pip 24.0/setuptools 65.5.0 from Python's bootstrap; the audit reported advisories for those installer tools, not the application's 13 runtime dependencies. The guide now updates to tested pip 26.2.1/setuptools 84.0.0 first. `py -3.11 -m pip_audit --path output/releases/samsarix-pilot-env/Lib/site-packages --progress-spinner off --format json --output output/playwright/release-041-pilot-audit-current.json` then reported **no known vulnerabilities among 15 public packages**, with the unpublished Samsarix package explicitly skipped. No application-security or future-advisory guarantee follows.
- Installed-wheel browser acceptance ran outside the checkout with `SAMSARIX_TEST_INSTALLED=1`, the absolute checkout `e2e` path, and `python -m pytest ... -o addopts= --browser chromium --browser firefox --browser webkit --tracing retain-on-failure --screenshot only-on-failure`. Initial result: **98 passed, one failed**. The trace proved the stale-delete test changed disk before its initial GET completed: the browser correctly received the new text/ETag. The test now waits for the old editor contents before the external write. Full rerun: **99 passed** in 367.07 seconds. No application behavior or assertion was weakened, and the original failure trace is retained locally.
- All eight CI jobs passed for both [initial candidate run 33413072882](https://github.com/Deathcharge/samsarix-workspace/actions/runs/33413072882) and [follow-up run 33413694387](https://github.com/Deathcharge/samsarix-workspace/actions/runs/33413694387). Initial Linux/Windows Python 3.13.15 bundles were downloaded and independently verified. They record GitHub's synthetic PR merge revision `0fafcce6ff3256df05a322da5711218ca90a5e4b` with tree `a3047ec597522ebc464c4fd4b7a4fbc95981a028`, identical to `cb26aec`'s tree. CI artifacts are evaluation outputs, not public package releases. The final documentation commit and post-merge revision must also pass the same eight gates; their observed results belong in [PR #15](https://github.com/Deathcharge/samsarix-workspace/pull/15) rather than a self-referential build stamp.

### Review, limitations, and disposition

Codex Security scan `3ee8d578-e004-4c1c-aedd-2b9103e8cd08` sealed successfully for immutable `2bebb9230b03551adb6aa60b5b0196b8c6fc8195..cb26aec4141175d7ae8a73c1cfe8115677d58329`, with **zero reportable findings**, **complete coverage of its seven authoritative source inventory entries**, and eight documented surface dispositions. All 22 changed paths were inspected, with shared writer/store/API/terminal callers followed as needed. A dedicated capability preflight and fresh-context architecture review were used; 135 model citation anchors were checked against 17 immutable files. The generated local report records the exact scope and exclusions. Later test/documentation-only changes were manually reviewed separately, not relabeled as part of that sealed scan. CodeRabbit skipped automatic review because of its repository-star threshold; its green status is not approval. TAC could not be verified because its connector was not connected; that advisory did not gate local review.

The independent architecture review clarified documentation: non-loopback bind/token-length enforcement is CLI-owned, while an embedded ASGI server owns its listener; document-root containment is not whole-process filesystem confinement; release-tool isolation/pins require the documented setup; ordinary bootstrap/pilot pip commands honor the caller's trusted-index configuration. The guides now make those boundaries explicit. No new network deployment, publisher identity or sandbox guarantee was inferred.

Disposition: **verified alpha release candidate for local evaluation**, not an externally validated competitive offering or public package release. The first-run reliability defects and reproducible evaluation-evidence gap are addressed. No known locally actionable P0 remains in the exercised core journey. External pilot consent, package-index ownership/publication authorization, signing/attestation decisions, rights review for commercial licensing, and physical macOS/Apple-device acceptance remain named gates. No money, telemetry, provider integration, production deployment, account creation, package publication or outreach was added. AGPL-3.0-only, Samsarix LLC attribution and the working contact/support addresses remain intact. Recovery still retains bounded unencrypted local data and is not backup or secure erasure. CI uses additional build time within existing jobs and bounded artifact retention, without runtime operating costs.

### Post-scan startup finding and remediation

The final adversarial check found a **release-blocking verifier defect after the scan above**: when a trusted `verify_release.py` was copied beside untrusted bundle files, Python imported neighboring `__future__.py`, `argparse.py`, or `hashlib/__init__.py` before reading the manifest. Each isolated disposable reproduction wrote a harmless marker; a clean control verified successfully. Exact-inventory rejection happened too late. This was an evaluation-tool issue, not a browser/API execution path. The sealed scan is preserved unchanged and must not be interpreted as evidence this defect was absent.

The narrow shared fix checks `sys.flags.isolated` for direct execution using only built-in `sys`, before every filesystem-backed import. It removes the unnecessary future import and requires `python -I` for build, verify, help, and copied-script invocations. Trusted imported helpers remain compatible with existing maintainer/tests. CI and both platform guides use the new command; the pilot's venv, pip, and application invocations also use `-I` from the bundle directory. No runtime dependency, command shell, privilege, publication, or environment-configuration change was introduced. No lint rule is suppressed.

Ordered local checks: `python -m compileall -q tools`, Ruff check/format, and Mypy passed first. `python -m pytest tests/test_release_evidence.py -k standalone -o addopts= --tb=short` showed **nine failures and two passing controls before the patch**. After the patch, `python -m pytest tests/test_release_evidence.py -o addopts=` passed **55 tests**, including all eleven cold-start cases. The original three diagnostics now return a useful nonzero isolation error without writing a marker. Isolated verification rejects shadow modules/packages as unlisted passive files without bundle mutation; clean verification/help and a `PYTHONPATH` startup-hook control pass. The trusted repository verifier also validates an existing seven-artifact bundle under `-I`. Fresh-context investigation preceded implementation; an independent bounded bypass/regression review and the final full-build/CI acceptance are recorded with their actual outcomes in [PR #15](https://github.com/Deathcharge/samsarix-workspace/pull/15).

Boundary: [Python isolated mode](https://docs.python.org/3/using/cmdline.html#cmdoption-I) excludes the script directory, user site-packages, and Python environment overrides; it is not OS confinement. The interpreter, its installed site environment, and verifier source must still be trusted. A script cannot undo startup hooks already executed before it, and trusted programmatic imports cannot retroactively isolate a caller. Unsigned hashes still do not authenticate a publisher. New clean post-fix bundles, exact-head eight-job CI, and post-merge CI remain the release handoff gates; source merge is not public publication.

Full source verification after the startup patch: `output/playwright/lifecycle-env/Scripts/python.exe -m pytest` passed **209 tests, one Windows FIFO skip**, with **92.54% application branch-aware coverage** in 90.44 seconds. Ruff check and formatting (26 files), Mypy (13 application/tool files), JavaScript syntax, and `git diff --check` also passed. The fresh bypass/regression reviewer found no concrete surviving route within the stated boundary and independently passed the 55 focused tests; Linux execution and the complete snapshot build were left to the final acceptance gates, not claimed by that reviewer. Application bytes are unchanged by this tooling fix; the earlier installed-wheel 99-case browser run remains applicable, and the final CI runs all four browser jobs again.
